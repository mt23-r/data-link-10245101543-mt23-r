...
统一入口为 `student_package/src_skeleton/run_all.py`，依次调用：M2解析与41字节TeachingLink编解码 -> M3批量解码、航迹、当前态势及SQLite逐字段回读/查询/航迹图选做 -> M4导出既有核验成果并使用代码内正式规则生成统一消息 -> M5四类必做规则与选做R5。
统一入口本身不清理 `student_package/output/`，但支持从空目录开始运行，不依赖任何既有输出。M6会把学校预生成候选和代码中固化的34条正式规则分别导出为 `llm_mapping_candidate.csv`、`verified_mapping_table.csv` 供审查，运行时直接使用代码规则完成映射，不重新执行候选核验、样例比较或报告生成。
该入口不运行手册6.6的OpenSky离线真实数据验证。手册6.6的入口在`student_package/src_skeleton/m3_opensky_real.py`。
...
- M3 默认假定输入二进制流的帧起始位置已经按 41 字节对齐；能够识别尾部不完整帧，但不负责在任意损坏字节流中重新搜索帧头。
- M4 的字段映射仅适用于课程给出的 OpenSky、TeachingLink 字段定义和统一模型，新增字段或协议版本变化后需要重新人工核验。
- M5 只执行手册规定的缺失、延迟、重复、航向越界及选做帧校验失败规则，不能判断数据来源真实性，也不包含复杂的多源融合和长期航迹质量评估。
- 6.6只有3个相邻真实数据快照，且与主线分开运行；不能评价实时接入与长期跟踪能力。

## 最终提交信息

- 仓库链接：https://github.com/mt23-r/data-link-10245101543-mt23-r
- 最终commit ID：将在提交后获取，登记到表格中
- 最后检查日期：2026-08-28
